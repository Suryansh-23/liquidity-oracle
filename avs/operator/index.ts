...
const oracleServiceManagerAddress =
  avsDeploymentData.addresses.oracleServiceManager;
...
const oracleServiceManagerABI = JSON.parse(
  fs.readFileSync(
    path.resolve(__dirname, "../abis/OracleServiceManager.json"),
    "utf8"
  )
);
...
const oracleServiceManager = new ethers.Contract(
  oracleServiceManagerAddress,
  oracleServiceManagerABI,
  wallet
);
...
const registerOperator = async () => {
  ...
  const operatorDigestHash =
    await avsDirectory.calculateOperatorAVSRegistrationDigestHash(
      wallet.address,
      await oracleServiceManager.getAddress(),
      salt,
      expiry
    );
...
const monitorNewTasks = async () => {
  ...
  oracleServiceManager.on(
    "NewTaskCreated",
    async (taskIndex: number, task: any) => {
      console.log(`New task detected: Hello, #${taskIndex}`);

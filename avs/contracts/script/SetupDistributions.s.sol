...
contract SetupDistributions is Script, Test {
    ...
    function setUp() public {
        ...
        oracleDeployment =
            OracleDeploymentLib.readDeploymentJson("deployments/oracle/", block.chainid);
        oracleConfig =
    ...
    function run() external {
        vm.startBroadcast(oracleConfig.rewardsInitiatorKey);
    ...
    function executeProcessClaim() public {
        ...
        vm.startBroadcast(deployer);
        earnerLeaves =
            _getEarnerLeaves(_getEarners(deployer), amountPerPayment, oracleDeployment.strategy);
        processClaim(
    ...
    function createAVSRewardsSubmissions(
        ...
        ERC20Mock(oracleDeployment.token).mint(
            oracleConfig.rewardsInitiator, _amountPerPayment * _numPayments
        );
        ERC20Mock(oracleDeployment.token).increaseAllowance(
            oracleDeployment.oracleServiceManager, _amountPerPayment * _numPayments
        );
        uint32 duration = rewardsCoordinator.MAX_REWARDS_DURATION();
        SetupDistributionsLib.createAVSRewardsSubmissions(
            oracleDeployment.oracleServiceManager,
            oracleDeployment.strategy,
            _numPayments,
            _amountPerPayment,
            duration,
            _startTimestamp

          ...
          try {
            ...
            switch (eventName) {
              ...
              case "ModifyLiquidity":
                const params = decoded as unknown as {
                  id: string;
                  sender: string;
                  tickLower: BigInt;
                  tickUpper: BigInt;
                ...
                if (perPoolEventMapping.has(params.id)) {
                  const eventMapping = perPoolEventMapping.get(params.id);
                  if (eventMapping) {
                    const events = eventMapping.get(eventName);
                    if (events) {
                      events.push(params);
                    } else {
                      eventMapping.set(eventName, [params]);
                    }
                  } else {
                    const newEventMapping = new Map<string, unknown[]>();
                  }
                } else {
                }
                break;
